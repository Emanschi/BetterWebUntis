...
**Abwesenheiten — Tab entfernt statt weiter offen gehalten (Nutzer-Feedback 2026-09-17):** `getTimetableWithAbsences` ist weiterhin gesperrt (Code -8509), und der Stundenplan kennt kein `lstype` für "Abwesenheit" — anders als bei Prüfungen gibt es hier kein äquivalentes Feld zum Umgehen. Die originale Oberfläche zeigt unter "Abwesenheiten" einen selbst meldbaren Eintrag mit Status "?" — das sieht nach einem separaten, in der 2018er-Doku nicht enthaltenen Workflow aus (Fehlzeiten-Selbstmeldung), nicht nach `getTimetableWithAbsences` (das ist für von der Schule/Lehrkraft erfasste Abwesenheiten gedacht). Kein dokumentierter Workaround gefunden. Auf Nutzerwunsch wurde der Tab komplett entfernt (`AbsencesScreen.tsx` gelöscht, Route/Nav raus) statt eine kaputte Fehlermeldung zu zeigen — die API-Methode selbst bleibt in `api/methods.ts`, falls sich das später ändert.

**Prüfungen — 2026-09-17 (zweite Runde): lstype "ex" bestätigt sich beim echten Server nicht.** Nach dem ersten Fix meldet der Nutzer: der Schuljahr-Filter funktioniert, aber es werden trotzdem keine Prüfungen angezeigt, obwohl laut Original-App z. B. am 18.09.2026, 12:20–13:10 (Fach NW2) eine stattfindet. `lstype: "ex"` war eine aus der Doku plausible, aber ungeprüfte Annahme — sie war offenbar falsch, oder die reale Schule nutzt dieses Feld anders/gar nicht. `scripts/smoke-test.ts` wurde um einen Diagnose-Block erweitert, der das ganze Schuljahr lädt, die `lstype`/`code`/`activityType`-Verteilung zählt und gezielt nach dem bekannten Termin (18.09.2026, ~12:20) sucht, um dessen volle Rohdaten zu zeigen. **Nächster Schritt:** Nutzer führt `npm run smoke` erneut aus, Ergebnis entscheidet über das weitere Vorgehen — entweder ein anderes Feld ersetzt `lstype === 'ex'` im Code, oder es bestätigt sich, dass die Original-App eine andere (undokumentierte) Quelle nutzt.

**Konsequenz für M11 (Kalenderabo):** hängt vom Ausgang der Prüfungen-Nachmessung ab — ob der Stundenplan-Workaround trägt oder nicht.
