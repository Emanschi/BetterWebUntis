...
- **Produktions-Proxy: gebaut, 2026-09-24 (`deploy/webuntis-proxy.php`).** Leitet gezielt alle fünf tatsächlich genutzten Pfade weiter (`jsonrpc.do` + die vier REST-Endpunkte aus R8/R11), nicht nur `jsonrpc.do` wie ursprünglich hier vermerkt. Zustandslos wie geplant (B1), feste Positivliste statt Prefix-Match. Noch **nicht** gegen den echten Server deployt/getestet — nur syntaktisch geprüft (kein `php -l` in dieser Umgebung verfügbar, Klammern-Balance manuell nachgezählt). Setup: `deploy/README.md`.
  **Erweitert, 2026-09-24 (IDEEN.md B11):** Ziel-Host ist nicht mehr fest `WEBUNTIS_HOST`, sondern kommt validiert (`*.webuntis.com`) aus dem Header `X-WebUntis-Host` — ein Deployment bedient jetzt jede WebUntis-Schule, nicht nur eine fest eingetragene. Dafür zusätzlich eine feste Route für die Schulsuche (`/WebUntis/schoolsearch` → `mobile.webuntis.com/ms/schoolquery2`, kein Login nötig). Der Real-Server-Deploy-Vorbehalt von oben gilt unverändert weiter.
